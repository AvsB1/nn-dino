...
def nn(nn_array, i_1, i_2, i_3): 
    ...
    w_i_h_1, w_h_1_2, w_h_2_o, b_i_h_1, b_h_1_2, b_h_2_o = nn_array
...
class Dino(pygame.sprite.Sprite):
    ...
    def __init__(self, x, x_a=0):
        ...
        if best_nn == []:
            self.nn = [
                np.random.uniform(-5, 5, (i,3)),
                np.random.uniform(-1, 1, (j,i)),
                np.random.uniform(-1, 1, (3,j)),
                np.zeros((i, 1)),
                np.zeros((j, 1)),
                np.zeros((3,1))]
        else:
...
while True:
    ...
    if obstacle_group.sprites() != []:
        close_obstacle = obstacle_group.sprites()[0]
        dino_distance = close_obstacle.rect.x - dino[0].x
    else: dino_distance = 1700

    ## obstacle code
    if pygame.time.get_ticks() - obstacle_timer >= obstacle_cooldown/time_multiplayer:
        obstacle = Obstacle(randint(0,3))
        obstacle_group.add(obstacle)
        obstacle_timer = pygame.time.get_ticks()
        obstacle_cooldown = randint(int(1600 - 20*game_speed),int(2700 - 20*game_speed))
