...
def create_dino(n=n_copies):
    dino.clear()
    #creating dino for the first time
    if generation == 1:
        for i in range(n):
            dino.append(Dino(x_ground, best_nn, random.randint(5,20), random.randint(5,20)))
            dino_group.add(dino[i])
    else:
        #creates 80% dinos with the same nn size as the best nn and 20% dinos with random genes and nn sizes
        for i in range(int(n*0.8)):
            dino.append(Dino(x_ground, best_nn, best_size[0], best_size[1]))
            dino_group.add(dino[i])
        for i in range(int(n*0.2)):
            dino.append(Dino(x_ground, [], random.randint(5,20), random.randint(5,20)))
            dino_group.add(dino[i+int(n*0.8)])
...
def nn(nn_array, i_1, i_2, i_3): 
    ...
    if (o[0]-o[1]) > 0.1:   return 2
    elif (o[1]-o[0]) > 0.1: return 2
...
class Dino(pygame.sprite.Sprite):
    ...
    def __init__(self, x, best_nn, h1_size, h2_size, x_a=0):
        super().__init__()
        self.x = x
        self.x_a = x_a
        ...
        if self.best_nn == []:
            self.nn = [
                np.random.uniform(-50, 50, (self.h1_size,3)),
                np.random.uniform(-0.5, 0.5, (self.h2_size,self.h1_size)),
                np.random.uniform(-0.5, 0.5, (3,self.h2_size)),
                np.zeros((self.h1_size, 1)),
                np.zeros((self.h2_size, 1)),
                np.zeros((3,1))]
        else:

            self.pre_nn = [
                np.random.uniform(-50*lr, 50*lr, (self.h1_size,3)),
                np.random.uniform(-0.5*lr, 0.5*lr, (self.h2_size,self.h1_size)),
                np.random.uniform(-0.5*lr, 0.5*lr, (3,self.h2_size)),
                np.random.uniform(-50*lr, 50*lr, (self.h1_size,1)),
                np.random.uniform(-0.5*lr, 0.5*lr, (self.h2_size,1)),
                np.random.uniform(-0.5*lr, 0.5*lr, (3,1))]

            self.nn = [best + r for best, r in zip(self.best_nn, self.pre_nn)]
    ...
    def update(self):
        global best_nn, dino_best, dino_distance, best_size
        ...
        for obstacle in obstacle_group:
            if self.rect.colliderect(obstacle.rect):
                best_nn = self.nn
                best_size = [self.h1_size, self.h2_size]
